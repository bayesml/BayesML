...
.. math::
    ...
    \alpha_{n,k}^{(t+1)} &= \alpha_0 + \frac{1}{2} N_k^{(t)}, \\
    \beta_{n,k}^{(t+1)} &= \beta_0 + \frac{1}{2} \left( -(\boldsymbol{\mu}_{n,k}^{(t+1)})^\top \boldsymbol{\Lambda}_{n,k}^{(t+1)} \boldsymbol{\mu}_{n,k}^{(t+1)} + \boldsymbol{y}^\top \boldsymbol{R}_k^{(t)} \boldsymbol{y} + \boldsymbol{\mu}_0^\top \boldsymbol{\Lambda}_0 \boldsymbol{\mu}_0 \right), \\
    \gamma_{n,k}^{(t+1)} &= \gamma_0 + N_{k}^{(t)}, \\
    \ln \rho_{i,k}^{(t+1)} &= \psi (\gamma_{n,k}^{(t+1)}) - \psi \left( {\textstyle \sum_{k=1}^K \gamma_{n,k}^{(t+1)}} \right) \nonumber \\
